          ...
          final child = (index == 0)
              ? _buildLoadingAnimator(
                  ...
                  child: _LoginCard(
                    ...
                    isSignUpEnabled: widget.isSignUpEnabled,
                    isForgotPasswordEnabled: widget.isForgotPasswordEnabled,
                  ),
                )
              : _RecoverCard(
                  emailValidator: widget.emailValidator,
...
class _LoginCardState extends State<_LoginCard> with TickerProviderStateMixin {
  ...
  List<Widget> getButtons(BuildContext context) {
    ...
    if (widget.isForgotPasswordEnabled)
      ws.add(_buildForgotPassword(theme, messages));
    ws.add(_buildSubmitButton(theme, messages, auth));
    if (widget.isSignUpEnabled)
      ws.add(_buildSwitchAuthButton(theme, messages, auth));
    return ws;
  }
